...
def modify_mytoday(shift_in_days:int)->datetime:
    # write a adjusted my_today in SuperPy.csv
            ...
            with open('Bought.csv', 'r', newline='') as csvfile:
                ...
                for row in csvreader:
                    if line_no >= 1:    #sla de eerste regel met kolomnamen over
                        ...
                        # bekijk of product verkocht is
                        bought_id_sold = my_functions.bought_id_sold(bought_id)
                        if bought_id_sold:
                            1 == 1 # a dummy code
                        else:
                            # bekijk of product verlopen is
                            if my_functions.bought_id_expired(bought_id):
                                # maak regel met gegevens en schrijf die schrijf weg naar Expired.csv
                                new_row=list()
                                ...
                                # maak regel met gegevens en schrijf die schrijf weg naar Inventory.csv
                                new_row=list()
                                new_row.append(buy_date.strftime("%Y-%m-%d"))
                                new_row.append(str(bought_id))
